...
from app.models import Casilla, Granja, Medicion, Parcela, Sensor, TipoCultivo, Usuario, UsuarioGranja
...
def _get_granja_for_user(granja_id: int, current_user: Usuario, db: Session) -> Granja:
    stmt = (
        select(Granja)
...
def get_granja(
    ...
    return _get_granja_for_user(granja_id, current_user, db)


@router.get("/{granja_id}/dashboard", response_model=GranjaDashboardRead, summary="Dashboard agregado de granja")
def get_granja_dashboard(
    granja_id: int,
    db: Session = Depends(get_db),
    current_user: Usuario = Depends(get_current_user),
) -> GranjaDashboardRead:
    _get_granja_for_user(granja_id, current_user, db)
...
def update_granja(
    ...
    granja = _get_granja_for_user(granja_id, current_user, db)
